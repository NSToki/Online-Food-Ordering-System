...
class Restaurant {
    ...
    public function getAllRestaurants() {
        $stmt = $this->conn->prepare(
            "SELECT * FROM restaurants WHERE is_open = 1 AND is_approved = 1 ORDER BY name ASC"
        );
        $stmt->execute();
    ...
    public function getMenuCategories($restaurant_id) {
        $stmt = $this->conn->prepare(
            "SELECT DISTINCT category FROM menu_items WHERE restaurant_id = :id AND is_available = 1"
        );
        $stmt->execute([':id' => $restaurant_id]);
